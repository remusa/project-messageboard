        ...
        suite('POST', function() {
            ...
            test('Posting new thread at /API/THREADS/{BOARD}', done => {
                chai.request(server)
                    ...
                    .end((err, res) => {
                        assert.equal(res.status, 200, 'Status OK')
                        const regexStr = new RegExp(`^http://${res.request.host}/b\/test`)
                ...
                chai.request(server)
                    ...
                    .end((err, res) => {
                        ...
                        assert.notProperty(
                            res.body[0],
                            'delete_password',
                            'Has not property delete_password'
                        )
            ...
            test('Delete a thread with incorrect id at /API/THREADS/{BOARD}', done => {
                chai.request(server)
                    ...
                    .end((err, res) => {
                        ...
                        assert.equal(
                            res.text,
                            'incorrect board or id',
                            'Incorrect is or board was entered'
                        )
                        done()
                    })
            })
                ...
                chai.request(server)
                    ...
                    .end((err, res) => {
                        ...
                        assert.equal(
                            ...
                            'Bumped_on updated after adding new post'
                        )
                        done()
                    })
            })
            ...
            test('Deleting post with invalid thread_id at /API/REPLIES/{BOARD}', done => {
                chai.request(server)
                    ...
                    .end((err, res) => {
                        ...
                        assert.equal(
                            res.text,
                            'incorrect board or thread id',
                            'Incorrect thread id was used'
                        )
                        done()
                    })
            })
